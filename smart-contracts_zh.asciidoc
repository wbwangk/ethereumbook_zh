...
应该谨慎使用 +delegatecall+ 调用。它可能会有一些意想不到的效果，特别是如果你调用的合约不是作为库设计的。

==== 事件

事件是便利生产交易日志的Solidity构造。当一个交易完成（成功与否）时，它会产生一个_transaction receipt_，就像我们在 <<evm>>  中看到的那样。交易收据包含_log_条目，用于提供有关在执行交易期间发生的操作的信息。事件是用于构造这些日志的Solidity高级对象。

事件在轻量级客户端和DApps中特别有用，它可以“监视”特定事件并将其报告给用户界面，或对应用程序的状态进行更改以反映基础合约中的事件。

事件对象将参数序列化并记录在区块链中的交易日志中。你可以在参数之前提供关键字 +indexed+ ，使得索引表（哈希表）的值可以由应用程序搜索或过滤。

到目前为止，我们还没有在我们的+龙头+示例中添加任何事件，所以让我们来做。我们将添加两个事件，一个记录任何提款，一个记录任何存款。我们将分别称这些事件+撤回+和+存款+。首先，我们在+水龙头+合约中定义事件：
...
我们选择将地址+索引为+，以允许在任何构建的用户界面中搜索和过滤以访问我们的+水龙头+。

接下来，我们使用 +emit+ 关键字将事件数据合并到交易日志中：

----
//给任何人提供以太的问题
函数提取（uint withdraw_amount）public {
    [...]
    msg.sender.transfer（withdraw_amount）;
    发出提取（msg.sender，withdraw_amount）;
}
//接受任何收到的金额
function（）public payable {
    发出存款（msg.sender，msg.value）;
}
----
...
首先，让我们看看一些文体规则，这些规则将帮助你编写一致且易于阅读的Solidity程序。这里有一个Solidity的风格指南：
...
在智能合约创建过程中需要考虑的两个最重要的概念是* gas *和* security *。

[[gas_sec]]
==== 气

* Gas​​ *在“gas”部分有更详细的描述，但在智能合约编程中是一个非常重要的考虑因素。燃料是决定用户分配给交易的计算能力的资源。如果在计算过程中超过了气体限制，则会发生以下一系列事件：

*抛出异常
*函数执行前的合约状态被恢复
*全部燃料作为交易手续费交给矿工，不予退还*

由于燃料由创建该交易的用户支付，因此不鼓励用户调用燃料成本高的功能。因此，程序员最大限度地减少合约职能的燃料成本。为此，在构建智能合约时建议采用某些做法，以尽量减少函数调用周围的燃料成本。

*避免动态大小的数组*

*通过动态调整大小的数组的任何循环，其中函数对每个元素执行操作或搜索特定元素都有气体溢出的风险。在找到所需结果之前，或在对每个元素采取行动之前，合约可能会用尽气体。

*如何估算合约方法的燃料？*

如果你需要估计执行某种合约方法所需的气体（考虑其调用参数），则可以使用例如以下程序;

[源码，JavaScript]
var contract = web3.eth.contract（abi）.at（address）;
var gasEstimate = contract.myAweSomeMethod.estimateGas（arg1，arg2，{from：account}）;

* gasEstimate *会告诉我们执行所需的气体单位数量。
...
METokenContract.web3.eth.getGasPrice（function（error，result）{
    ...
    //获取合约实例
    METokenContract.deployed（）。然后（函数（METokenokenTractInstance）{

        //在函数名称后面使用关键字'estimateGas'来获得这个特定函数的气体估计（aprove）
        返回METokenContractInstance.aprove.estimateGas（_address，100）;

    }）。then（function（result）{
        var gas = Number（result）;

        console.log（“gas estimate =” +gas+ “units”）;
...
随着区块链处于早期阶段，*安全*是编写智能合约时最重要的考虑因素之一。与其他程序一样，智能合约将执行完全写入的内容，这并不总是等同于程序员的意图。为此，程序员必须了解常见的安全漏洞和适当的方法来防范这些漏洞。以下列出了一些过去出现的安全问题。

*重入*

重入是编程中的一种现象，在该编程中，函数或程序被中断，然后在先前调用完成之前再次调用。在智能合约编程的情况下，当合约A调用合约B中的一个函数时，可能会发生重新合并，合约B又调用合约A中的相同函数，导致递归执行。在合约状态在关键性呼叫结束后才更新的情况下，这可能是特别危险的。

为了理解这一点，设想一个用户拨打银行合约的提款。用户A调用银行B中的撤销功能，该功能执行以下操作：

//“用户A”建议使用EOA账户，但在此示例中它是合约。

。检查A是否有可用余额
。拨打A的默认功能，在乙太银行支付A
。更新用户A在合约中的余额

作为附注，在支付期间调用A的默认功能的原因是合约B允许A在该支付期间执行代码。例如，如果合约A保留了它所支付的资金数量，则可能需要更改名为“余额”的变量，将“余额”设置为与之前的金额相等，再加上刚刚支付的金额。

但是，恶意攻击者可以利用这种执行。想象一下，在A的默认功能中，用户A再次调用银行B的退出功能。B将首先检查A是否有可用余额，但由于步骤3（其更新A的余额）尚未执行，所以对于B银行来说，用户A仍然有可用资金撤回，无论此次数多少次函数被重新调用。因此，只要有可用于执行的气体，就可以调用“撤回”。

这个漏洞因其在DAO攻击中的相关性而特别出名。用户利用了这样一个事实，即在呼叫转移资金并撤回价值数百万美元的乙醚后，合约中的余额发生了变化。

为了防止重新入侵，http://solidity.readthedocs.io/en/v0.4.21/security-considerations.html[Solidity]建议程序员遵守Checks-Effects-Interactions模式，其中a函数调用（如减少余额）发生在拨打电话之前。在我们的例子中，这意味着切换步骤3和2：在付出之前更新用户的余额。在以太坊，这是完全没问题的，因为交易的所有影响都是原子的，这意味着在没有用户支付的情况下更新余额是不可能的。要么都发生，要么抛出异常，也不会发生。这防止了重新入侵攻击，因为所有后续调用原始合约都会遇到正确的修改状态。
...
任何编程范例的软件开发人员通常都会遇到以行为，结构，交互和创建为主题的重复设计挑战。通常这些问题可以概括
并重新应用于未来类似性质的问题。当给定正式结构时，这些概括称为*设计模式*。
智能合约有自己的一系列重复出现的设计问题，可以使用下面描述的一些模式来解决。

在智能合约的发展中存在着无数的设计问题，因此无法讨论所有这些问题
这里。因此，本节将重点讨论智能合约设计中最常见的三类问题分类：*访问控制*，*国家流量*和*资金支出*。

在本节中，我们将制定一份合约，最终将包含所有这三种设计模式。该合约将运行一个投票系统
允许用户对“真相”进行投票。该合约将提出一项索赔，例如“小熊队赢得世界系列赛”。或者“纽约市正在下雨”，然后用户会有
有机会选择真或假。如果大多数参与者投票赞成真实，并且同样如果多数投票，合约将认为该提议是真实的
的参与者投了赞成票。为了激励真实性，每次投票必须向合约传送100张以上的唱片，而失败少数派出的资金将分成几份
大多数。大多数参与者将从少数人中获得他们的部分奖金以及他们的初始投资。
...
Solidity功能修饰符提供了一个简洁的方式来实现这些限制。
...
*修饰符和函数的说明：*
 ...
 -  * onlyOwner *：这个修饰符可以修饰一个函数，使得函数只能被发送者调用，地址与* owner *匹配。
 -  * onlyVoter *：这个修饰符可以修饰一个函数，使得函数只能被注册选举人调用。
 -  * addVoter（选民）*：此功能用于将选民添加到选民列表。该功能使用* onlyOwner *修饰符，因此只有该合约的所有者可以调用它。
 -  * vote（val）*：这个函数被投票者用来对所提出的命题投下真或假。它用* onlyVoter *修饰符装饰，所以只有已登记的选民可以打电话给它。

[[state_flow_sec]]
===== 状态流

许多合约将需要一些操作状态的概念。合约的状态将决定合约的行为方式以及它提供的操作
在给定的时间点。让我们回到我们的真实投票系统来获得更具体的例子。

我们投票系统的运作可以分为三个不同的州。

。*注册*：服务已创建，所有者现在可以添加选民。
。*投票*：所有选民投票。
。*以太坊*：投票付款被分开并发送给大多数参与者。
...
合约TruthVote {
    ...
    修饰符isCurrentState（States _stage）{
        ...
        转变前的（）
    {
        ...
        for（uint j = 0; j <winningGroup.length; j+ +）{
            winningGroup [j]的.transfer（winningCompensation）;
        }
    }
}
----
...
*修饰符和函数的说明：*
 ...
 -  * isCurrentState *：在继续执行装饰函数之前，此修饰符将要求合约处于指定状态。
 -  * pretransition *：在执行装饰函数的其余部分之前，此修饰符将转换到下一个状态
 -  * goToNextState *：将合约转换到下一个状态的功能
 -  *以太坊*：计算多数并相应地以太坊奖金的功能。只有主人可以调用这个函数来正式结束投票。
 -  * startVote *：所有者可用于开始投票的功能。

注意到允许所有者关闭投票流程可能会很重要，因为这会打开本合约，直至滥用。在更真实的实现中，投票期应在公众理解的时间段后结束。为了这个例子，这很好。

现在增加的内容确保只有在所有者决定开始投票期间时才允许投票，用户只能在投票前由业主注册，并且在投票结束后才能分配资金。

[[withdraw_sec]]
===== 撤回

许多合约将为用户从中提取资金提供一些方法。在我们的工作示例中，大多数用户在合约时直接发送货款
开始以太坊资金。虽然这看起来有效，但它是一种思想不足的解决方案。在* disperse *中* addr.send（）*调用的接收地址可以是一个合约
具有故障回退功能，并因此破坏*以太坊*。这有效地阻止了所有更多的参与者接收他们的收入。
更好的解决方案是提供一个用户可以打电话来收取收入的提款功能。
...
*修饰符和（更新）功能的说明：*
 ...
 -  *确定*：此功能与以前的*以太坊*功能非常相似，除了现在只计算赢家和获胜赔偿，并且实际上并未发送任何资金。
 -  *投票*：投票现在被添加到投票映射和真/假计数器递增。
 -  *撤回*：允许选民收取奖金（如果有的话）。



这样，如果发送失败，它只会在特定的来电者的情况下失败，并且不会妨碍所有其他用户收取他们的奖金的能力。
...
https://blog.zeppelin.solutions/onward-with-以太坊-smart-contract-security-97a827e47702

https://medium.com/zeppelin-blog/the-hitchhikers-guide-to-smart-contracts-in-以太坊-848f08001f05#.cox40d2ut

也许最基本的软件安全原则在于最大化可信代码的重用。在区块链技术中，这甚至会凝结成一句格言：“不要推出自己的密码”。就智能合约而言，这意味着尽可能多地从经过社区彻底审查的免费图书馆中获利。

在以太坊中，使用最广泛的解决方案是https://openzeppelin.org/[OpenZeppelin]套件，这是一个从ERC20和ERC721标记的实现到众多众包模型到简单行为常见于“Ownable”，“Pausable”或“LimitBalance”等合约中。该存储库中的合约已经过广泛的测试，并且在某些情况下甚至可以用作_de facto_标准实现。它们可以免费使用，并且由https://zeppelin.solutions [Zeppelin]和不断增长的外部贡献者列表构建和修复。

同样来自Zeppelin的是https://zeppelinos.org/[zeppelin_os]，一个用于安全地开发和管理智能合约应用程序的服务和工具的开源平台。zeppelin_os在EVM之上提供了一个层，使开发人员可以轻松启动与可自行升级的经过良好测试的合约链上库链接的可升级DApp。这些库的不同版本可以共存于区块链中，并且凭证系统允许用户在不同方向上提出或推动改进。该平台还提供了一套用于调试，测试，部署和监控以太坊式应用程序的脱链工具。


[[eth_contract_abi_sec]]
=== 以太坊合约应用程序二进制接口（ABI）

在计算机软件中，应用程序二进制接口（ABI）是两个程序模块之间的接口; 通常，一个在机器代码级别，另一个在用户运行的程序级别。ABI定义了如何在*机器码*中访问数据结构和计算例程; 这不会与API相混淆，它以高级的，通常是人类可读的格式将此访问定义为*源代码*。因此，ABI是编码和解码数据进出机器码的主要方式。

在以太坊，ABI用于编码EVM的合约呼叫，并从交易中读取数据。正如前面部分所讨论的，以太坊智能合约是在*合约地址*下部署在区块链中的字节码。ABI的目的是为了指定合约中的哪些函数被调用并获得函数将以预期格式返回数据的保证。

____
如果账户或Web应用程序想要与已发布的合约进行交互并使用其功能之一，则账户首先需要通过ABI对函数的定义进行散列，以创建* EVM字节码*。然后，为了调用这个函数，这个账户会把这个字节码传递给一个交易的数据字段，这样字节码就可以用合约地址处的代码来解释。因此，外部函数调用的两个必要信息片段将是* ABI *和合约的地址，其中函数被写入*。我们在下面的详细例子中演示ABI的使用。
____

合约的ABI的JSON格式由一组函数和事件描述给出。函数描述是一个JSON对象，它包含`type`，`name`，`inputs`，`outputs`，`constant`和`payable`字段。事件描述对象具有`type`，`name`，`inputs`和`anonymous`的字段。

因此，ABI指定了关于智能合约中的功能的信息，中继诸如输入和类型的信息。但是，ABI * only *包含有关函数和事件的信息，这意味着它不会包含状态变量或修饰符等字段的值。
...
*前四字节*：通话数据始终以功能签名的* 4个字节*开头。具体来说，这些是函数的*签名*的Keccak-256散列的前四个字节。在这种情况下，签名只是一个带有括号的参数类型列表的函数名称，由单个逗号分隔。

假设，作为一个运行的例子，简短的合约`Test`：

[源，坚固]
合约测试{
  函数foo（uint32 u，bool b）返回（bool b）{u> 2000 && b; }
}

。要编码函数`foo（uint32 u，bool b）`：
*'foo（uint32 u，bool b）'的签名是'foo（uint32，bool）`
*通过Keccak-256运行`foo（uint32，bool）`输出哈希`cc822237a37f9290b70dab4d640156d816bf8abdb959b5971d803a639dadef98`
*哈希的前四个字节是“cc822237”，它使我们得到一个方法ID“0xcc822237”
*在Go 以太坊（geth）JavaScript控制台中执行命令web3.sha3（“foo（uint32，bool）”）。substring（0，10）`产生结果`“0xcc822237”`

*第5字节和开*：我们通过编码函数的*参数来继续我们的数据。对uint32进行编码意味着将数字转换为十六进制格式并用0填充，直到长度变为32字节为止，而编码“bool”相当于编码“uint”，其值为“true”和“0” `为`错误`。完整的类型编码列表可以在_Further Reading_部分的链接中找到。
...
在以太坊 ABI上可以找到更严格和更深入的解释
`的https：// solidity.readthedocs.io/EN /开发/ ABI-spec.html`。
...
松露测试::松露框架的一部分，松露允许用Javascript（基于Mocha）或Solidity编写单元测试。这些测试是针对TestRPC/Ganache运行的。编写这些测试的更多细节位于<<松露>>

////
TODO：为“松露”添加锚点
////

Embark框架测试:: Embark与Mocha集成以运行用JavaScript编写的单元测试。测试依次运行在TestRPC/Ganache上部署的合约。Embark框架自动部署智能合约，并在合约更改时自动重新部署合约。它还跟踪部署的合约，并在真正需要时部署合约。Embark包含一个测试库，可以在EVM中快速运行和测试你的合约，并具有类似```assert.equal（）```的功能。```````````````````````````````````````````````````````“`````````

DApp::  DApp使用本地Solidity代码（一个名为ds-test的库）和一个Parity构建的Rust库（称为Ethrun）执行以太坊字节码，然后声明正确性。ds-test库提供用于验证控制台中数据记录的正确性和事件的断言功能。
...
assertEq0（字节a，字节b）
expectEventsExact（地址目标）
....

记录事件会将信息记录到控制台，使其对调试有用。
....
日志（字节）
log_bytes32（bytes32）
log_named_bytes32（bytes32 key，bytes32 val）
log_named_address（bytes32键，地址val）
log_named_int（bytes32 key，int val）
log_named_uint（bytes32 key，uint val）
log_named_decimal_int（bytes32键，int val，uint小数）
log_named_decimal_uint（bytes32键，uint val，uint小数）
....

Populus::  Populus使用python和它自己的链模拟器来运行用可靠编写的合约。单元测试是用pytest库编写的。Populus支持专门用于测试的书面合约。这些合约文件名应该与glob模式```Test * .sol``匹配，并且可以位于项目测试目录```。/ tests /```下的任何位置。

| =======
|框架| 测试语言| 测试框架| 链仿真器| 网站
|松露| Javascript/Solidity | 摩卡| TestRPC/Ganache | truffleframework.com
|登船| Javascript | 摩卡| TestRPC/Ganache | embark.readthedocs.io
| DApp | 坚实| ds-test（自定义）| Ethrun（奇偶校验）| dapp.readthedocs.io
|杨树| Python | Pytes | Python链模拟器| populus.readthedocs.io
| =======
...
eth.getPastLogs（选项）
....
获取位于地址的合约的完整日志，在选项中指定。这有助于查看合约呼叫的历史记录。
....
eth.getStorageAt（地址，位置）
....
获取位于+地址+处的存储，其中偏移量为+位置+显示存储在该合约中的数据。
